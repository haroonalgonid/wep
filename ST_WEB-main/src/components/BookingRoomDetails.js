...
function BookingRoomDetails() {
    ...
    const fetchBookingDetails = async () => {
      try {
        ...
        const response = await fetch(
          `https://backend-fpnx.onrender.com/hotels/bookings/${id}`,
          {
            method: "GET",
            headers: {
  ...
  const updateStatus = async (newStatus) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://backend-fpnx.onrender.com/hotels/hotel-bookings/${id}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updatePaymentStatus = async (newPaymentStatus) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://backend-fpnx.onrender.com/hotels/hotel-bookings/${id}/payment-status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="flight-details" dir="rtl">
      ...
      <div className="details-view">
        <div className="details-section">
          ...
          <p><strong>تاريخ الوصول:</strong> {new Date(booking.checkInDate).toLocaleDateString('ar-SA')}</p>
          <p><strong>تاريخ المغادرة:</strong> {new Date(booking.checkOutDate).toLocaleDateString('ar-SA')}</p>
        ...
        <div className="details-section">
          ...
          {booking.room ? (
            <>
              <p><strong>نوع الغرفة:</strong> 
                {booking.room.type === 'single' ? 'غرفة فردية' : 
                 booking.room.type === 'double' ? 'غرفة مزدوجة' : 
                 booking.room.type === 'suite' ? 'جناح' : booking.room.type}
              </p>
              <p><strong>سعر الليلة:</strong> {booking.room.pricePerNight} $</p>
              <p><strong>الوصف:</strong> {booking.room.description || 'لا يوجد وصف'}</p>
            </>
          ) : (
            <p className="error-message">معلومات الغرفة غير متوفرة</p>
          )}
        </div>
